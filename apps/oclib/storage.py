...
class StorageRedis(Storage):
    ...
    def hgetall(self, _class, key):
        key = str(self._get_cls_key(_class, key))
        all_value = {}
        value = self.redis_list[self._hash(key)].hgetall(key)
        if not value:
            return None
        for v in value:
            all_value[v] = msgpack.loads(value[v], encoding='utf-8')
        return all_value

    def hget_user_model(self, _class, key, field):
        cls_key = str(self._get_cls_key(_class, key))
        field = str(field)
        value = self.redis_list[self._hash(cls_key)].hget(cls_key, field)
        if not value:
            return None
        value = msgpack.loads(value, encoding='utf-8')
        return _class.load(value)

    def hset_user_model(self, obj):
        key = str(self._get_obj_key(obj))
        field = str(self._hget_obj_field(obj))
        value = self._to_msgpack(obj)
        redis = self.redis_list[self._hash(key)]
        return redis.hset(key, field, value)

    def _hget_obj_field(self, obj):
        return '%s' % (str(getattr(obj, obj.opk)))

    def hdelete_user_model(self, obj):
        key = str(self._get_obj_key(obj))
        field = self._hget_obj_field(obj)
        redis = self.redis_list[self._hash(key)]
