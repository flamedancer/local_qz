...
def banquet_info(rk_user, params):
    ...
    * 获取探索的相关内容
    ...
    #获取探索的相关内容
    explore_info = get_explore_info(rk_user.uid)
...
def get_banquet_stamina(rk_user,params):
    ...
    #根据点击次数获取可以领取多少体力
    click_num = int(params['click_num'])
    ...
    ul.banquet_info[str(start_time)] = True
    ul.put()
...
def explore(rk_user,params):
    ...
    *输入 要探索的宝藏类型  探索次数
    ...
    if use_coin:
        ...
        #减元宝
        rk_user.user_property.minus_coin(all_cost_coin,'explore_type:%s_allcost:%s'%(shovel,all_cost_coin))
    else:
        #将类型转化为可识别的道具
        props_config = game_config.props_config
        #根据探索的类型获取探索所需要的道具
        ...
        #减素材
        user_pack_obj.minus_props(p_id,all_cost_num)
    #减可探索的次数
    # user_activity_obj.min_explore_times(shovel,all_cost_num)
    explore_info = add_explore_info(shovel,rk_user.uid,times)
    return 0,{'explore_info':explore_info}


def add_explore_info(explore_type,uid,times):
    '''
    根据探索类型获取探索的内容 并且添加
    '''
    returndata = {}
    ...
    for i in xrange(times):
        ...
        #格式化要获取的 id 和权重
        wlst = []
        for explore_id in explore_info:
            weight = int(explore_info[explore_id]['weight'])
            wlst.append([explore_id,weight])
        #获取探索得到的 id  #[[value,weight],[value,weight],[value,weight],[value,weight]]
        get_id = utils.windex(wlst)
        #获取探索得到的数量
    ...
    for g_type in data:
        if g_type == 'mat':
            ...
            real_type = g_type.replace('_soul','')
